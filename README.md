...
[![GitHub repo](https://img.shields.io/badge/GitHub%20Repo--ffffff?style=social&logo=github)](https://github.com/aydwi/barrel)
...
Barrel exposes a [well documented](https://barrel.wiki/barrel_8h.html), succinct C++ API which can be integrated with a wide variety of libraries, tools and frameworks. Find examples later.
...
int main(int argc, char** argv) {
    ...
    BrewCommand<BrewCommandType::Builtin> cmd(BrewCommandType::Builtin::INFO);
    cmd.execute();
...
1. Provide a performant, robust and type-safe API to interact with Homebrew on macOS — Core commands supported in the initial release with an increasing support for commands in each subsequent release.

2. Maintain portability — Work with several toolchains, avoid external dependencies.

3. Easy to work with — Reasonable defaults, plentiful overrides, intuitive exception handling.

4. Track upstream Homebrew releases closely — Quick API updates when something breaks.

5. Establish a high quality codebase; make effective use of facilities provided by modern C++ standards (11 and above, but focus on 20 and 23).

6. Provide high quality documentation of the API and library internals; quality over quantity.

7. Serve as a learning mechanism for the author — Intermediate/advanced C++ techniques, library authorship and maintenance.

8. Serve as a learning mechanism for new C++ programmers and the community — Reading library code should be pleasant, helpful "cues" for a variety of techniques.

#### Non-goals

1. Provide a comprehensive API to the entirety of Homebrew at all times; Homebrew is enormous, I am a graduate student and work on Barrel in my spare time. API support will be on a best-effort and community recommendation basis.

2. Support Linuxbrew or non-Apple platforms.

#### Undecided (as of now)

1. Provide versioned APIs for different major releases of Homebrew.
...
3. Link to Barrel from your project
     ...
     * #### <ins>As an external dependency</ins>
          ...
          Say you have a project you want to include Barrel in, but you do not want to add Barrel to your project's source tree. In this case, you can include Barrel as an external dependency after building it. [`find_package`](https://cmake.org/cmake/help/latest/command/find_package.html#find-package) makes it starightforward.
     
          I have included a simple CMakeLists.txt example you can consult at [`examples/external/CMakeLists.txt`](https://github.com/aydwi/barrel/blob/master/examples/cmake/external/CMakeLists.txt).
